...
curl -L ${DOWNLOAD_URL} -o $TMP_DIR/vscodium.deb
...
# Vagrant
LATEST_VERSION=$(curl --silent https://www.vagrantup.com/ | grep -oh "Download [0-9]\{1\}.[0-9]\{1\}.[0-9]\{1\}" | cut -d" " -f2)
curl --silent -L https://releases.hashicorp.com/vagrant/${LATEST_VERSION}/vagrant_${LATEST_VERSION}_linux_amd64.zip -o $TMP_DIR/vagrant.zip
pushd $TMP_DIR
